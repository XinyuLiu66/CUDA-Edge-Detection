...
void cuda_grayscale(int width, int height, float *image, float *image_out)
{
    ...
    // Calculate the row index of the image element and image_out
    ...
    float *pixel = &image[offset + Col * 3];

    // Convert to grayscale following the "luminance" model
    image_out[offset_out + Col] = pixel[0] * 0.0722f + // B
    pixel[1] * 0.7152f + // G
    pixel[2] * 0.2126f;  // R
...
void gaussian(int width, int height, float *image, float *image_out)
{
    ...
    for (int h = 0; h < (height - 2); h++)
    {
        ...
        for (int w = 0; w < (width - 2); w++)
        {
            ...
            //printf("image_out[%d] = %f\n", offset + (w + 1), image_out[offset + (w + 1)]);
        }
...
void cuda_gaussian(int width, int height, float *image, float *image_out)
{
    ...
    //if (Row < 100 && Col < 100) {
    if(Row < height - 2) {
        int offset_t = Row * width;
        int offset   = (Row + 1) * width;

        if(Col < width - 2) {
            image_out[offset + (Col + 1)] = cuda_applyFilter(&image[offset_t + Col], width, gaussian, 3);
        }
    }
}
...
void cuda_sobel(int width, int height, float *image, float *image_out)
{
    ...
    if(Row < height - 2) {
        ...
        if(Col < width - 2) {
            ...
            float gx = cuda_applyFilter(&image[offset_t + Col], width, sobel_x, 3);
            float gy = cuda_applyFilter(&image[offset_t + Col], width, sobel_y, 3);
            image_out[offset + (Col + 1)] = sqrtf(gx * gx + gy * gy);
...
int main(int argc, char *argv[])
{
    ...
    // Aufgabe 1
    int grid_width = bitmap.width/BLOCK_SIZE;
    int grid_height = bitmap.height/BLOCK_SIZE;
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        cudaMemcpy(d_image_out[i], image_out[i], image_size, cudaMemcpyHostToDevice);
    }
    ...
    cudaMalloc((void**) &d_bitmap, image_size * sizeof(float));
  
    //TODO (2 pt): Copy input image into the device memory

    cudaMemcpy(d_bitmap, bitmap.data, image_size, cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(image_out[0], d_image_out[0], image_size, cudaMemcpyDeviceToHost);
    ...
    cuda_gaussian<<<grid, block>>>(bitmap.width, bitmap.height, d_image_out[0], 
        d_image_out[1]);
    ...
    t[1] = get_clock_time();
    //TODO (2 pt): transfer image from device to the main memory for saving onto the disk
    cudaMemcpy(image_out[1], d_image_out[1], image_size, cudaMemcpyDeviceToHost);
